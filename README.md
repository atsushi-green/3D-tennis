...
| B / V / C | 押しっぱなしでテイクバックを溜め、離すと打つ（B＝フラット／V＝トップスピン／C＝スライス。押したキーがそのまま結果になり、当たるまで押し続ける必要はない。クリック/タップでも可＝フラット）。サーブは1回目が即トス、トス中の2回目から溜められる。溜め具合は画面下のバーに表示。グラウンドストローク・サーブ以外（スマッシュ・ボレー・CPU/AIの打球）はどのキーでもフラット固定 |
| Shift | ロブ |
