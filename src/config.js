...
(function (RallyOne) {
  ...
  const SERVE = {
    ...
    // CHARGE_SWEET_T は Space を押してから離すまでの理想の保持時間（秒）。トスの頂点到達
    // （約0.40秒、TOSS_PEAK/BALL_Y/PHYSICS.GRAVITY から決まる）を少し過ぎた、実際の
    // サーブと同じ「打点は頂点よりわずかに落ち始めたタイミング」に合わせてある。
    // CHARGE_WINDOW はそこから離れたとき威力が0まで落ちる片側の幅（秒）。
    // トスの全滞空時間（約0.79秒）より内側に収まるようにしておくこと（さもないと
