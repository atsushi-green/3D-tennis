...
(function (RallyOne) {
  ...
  const SWING = {
    ...
    GROUND_START: -1.45, // フォアハンドのテイクバック（ここからバックハンドは体の反対側に鏡映しになる）。
                         // REST_Y との差を大きめに取り、構えているだけかテイクバック中かが一目で分かるようにする
    GROUND_SWEEP: 2.4,   // 振り抜く角度
    // Space を溜めている間、テイクバックがどこまで深く入るか(rad)。溜め量(0〜1)に比例して
    // GROUND_START からさらに引き、離した瞬間のスイングはその位置から始まって同じ弧を振り抜く
    // （溜めるほど弧も広がる＝強打ほど大きく振っているように見える）。
    CHARGE_PULL: 0.55,
