...
(function (RallyOne) {
   ...
   * 打つほど「流れる」。振り始め（溜めキーを離す）が早いほど体の前で当たるので、
   * 「早く振る＝引っ張り／引きつける＝流し」がそのまま出る。
   * 流し側が出しにくかったので、NEUTRAL_REL（ずれない打点）は「打てる区間のやや前寄り」
   * に置いてある：ボールが手の届く範囲に入った瞬間に振れば引っ張り、半分ほど引きつけて
   * 打てば流しになる。
   */
  const TIMING_AIM = {
    // 打点のタイミングは「打てる区間（リーチの円をボールの通り道が横切る弦）のどこで
    // 捉えたか」で測る（game.js#contactTiming）。+1＝円に入った瞬間＝体のいちばん前、
    // 0＝真横、-1＝抜ける直前まで引きつけきった、の正規化した値。
    // 生の前後差(m)で測っていた頃は、同じ振り方でも球の左右のずれで弦の長さが変わるぶん
    // 結果がばらつき（実測：同じ振り方で「体の前」〜「引きつけきり」まで散った）、
    // 狙って引っ張り／流しに打ち分けられなかった。
    // 引っ張り側と流し側で帯の広さを変えてある。人間は「手が届くより少し早く」振るのが
    // 自然で、実測でも打点は 0.93〜0.99（＝ほぼ円の縁）に固まる。ここを基準にすると
    // 引っ張りは黙っていても出るが、流しは「縁を過ぎてから」の数フレームを狙い撃たねば
    // 出せない＝事実上打ち分けられない（ユーザー報告「流し方向が難しく、正面にしか
    // 飛ばせない」）。そこで NEUTRAL_REL を縁の近くまで前に出し、流し側の帯を広く取る：
    // 手が届くより早く振れば引っ張り、届いてから引きつけて打てば流し、になる。
    NEUTRAL_REL: 0.80, // ここで捉えるとずれない（＝狙ったところへ真っすぐ）
    PULL_BAND: 0.20,   // NEUTRAL_REL よりこれだけ前で捉えると引っ張りが最大になる
    FLOW_BAND: 0.55,   // 同、これだけ引きつけると流しが最大になる
