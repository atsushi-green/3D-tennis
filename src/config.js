...
(function (RallyOne) {
  ...
  const CPU = {
    ...
    // （CPU.SMASH_LOB_PEAK=3.0）を超え、人間側のスマッシュ案内（ノーバウンドで叩ける
    ...
    // 「上から落ちてきて帯に入る」だけを条件にすると、普通の深いドライブも自陣の中盤で
    // 2m 台を降りてくるので全部スマッシュ対象になってしまう（実測でAIの打球の1/4）。
    // ロブ（CPU.LOB_T / SHOT.LOB_T の山なり）と、浮いた緩い球だけをここで選り分ける。
    SMASH_LOB_PEAK: 3.0,
    ...
    SMASH_STRETCH_T: 0.80,   // 追い込まれて打つスマッシュはここまで威力が落ちる
