...
(function (RallyOne) {
  ...
  const TIMING_AIM = {
    ...
    // ←→ の狙い（SHOT.AIM_X＝2.7m）に足したとき、サイドライン（HALF_W＝4.115m）の内側に
    // 収まる大きさにしてある。←→ は「動く方向」と「打つ方向」を兼ねているので、ボールを
    // 追いながら打つとほぼ常に狙いが入った状態になる：ここが大きすぎると、狙いと同じ側へ
    // ずれたときに必ずサイドアウトし（実測で8割）、逆にずれたときは狙いを打ち消して
    // ど真ん中へ飛ぶ（＝以前の「正面にしか飛ばない」）。左右どちらへ打つかは方向キーで
    // 決め、タイミングは「どれだけ角度をつけるか」を足し引きする、という関係にする。
    MAX_SHIFT: 1.30,       // 最大でどれだけ狙いが横にずれるか(m)
