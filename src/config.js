...
(function (RallyOne) {
  ...
  /**
   * 必殺技。スタート画面で好きなだけ選んでおくと、試合中は**条件を満たした1打で自動的に**
   * 発動する（専用の操作はない。いつも通り溜めて離すだけ）。同時に複数の条件を満たした
   * ときは、SPECIAL_MOVES の並び順（＝優先度）が上のものひとつだけが出る。
   * 使える回数は **技ごとに** USES_PER_GAME 回／1ゲーム（技どうしで融通はしない。
   * ゲームが替わると全部回復する）。使い切った技は候補から外れ、同じ場面でも次の
   * 優先度の技が拾う（game.js#pickSpecial）。
   *
   * 「どの技が出るか」は SPECIAL_MOVES の並び順（＝優先度）で決まり、実際の条件判定は
   * game.js#specialMatch にある（ここは数値だけを持つ）。技ごとの小オブジェクトの名前は
   * SPECIAL_MOVES の key を大文字にしたものと対応させてある。
   */
  const SPECIAL = {
    USES_PER_GAME: 1,        // **技ごとに** 1ゲームに使える回数（技どうしで融通はしない）
    // タイブレーク中は「ゲーム」が進まないので、この本数ごとに回数を回復させる
    // （そうしないと 6-6 以降の長いタイブレークで一度も使えなくなる）。
    TIEBREAK_REFRESH_POINTS: 6,
    CALL_T: 1.1,             // 技名のコールを出しておく時間(秒)
    IMPACT_POWER: 1,         // 必殺技の打球はフル溜め扱いの演出（着弾フラッシュ・膨張）にする

    /** キックサーブ：高く跳ねてレシーバーを押し下げるトップスピンサーブ。 */
    KICK: {
      // ネットのかなり上を通す（＝落差で跳ねる）。通常のサーブは SERVE.CLEARANCE。
      // ここを上げるほど山なりになり、solveShot() が滞空を伸ばすぶん球速が落ちる：
      // 実測（線ぴったりのフル溜め）で 0.5→169km/h・跳ね1.37m、0.65→134km/h・1.50m、
      // 0.95→107km/h・1.67m（フラットの普通のサーブは 201km/h・跳ね0.93m）。
      // 「速さは普通のサーブに譲るが、肩の高さまで跳ね上がる」ところを狙って 0.65 にした。
      CLEARANCE: 0.65,
      T_MULT: 1.0,           // さらに滞空を伸ばしたいとき用の倍率（1.0＝素のまま）
      DEPTH: 0.35,           // サービスラインからこれだけ手前＝ボックスのいちばん深いところ
      BOUNCE_MULT: 1.28,     // 着地後の弾み（reflectBounce が返した vy に掛ける）
    },

    /** ダンクスマッシュ：打点の上限が上がり、真下へ叩き落とす最速の決め球。 */
    DUNK: {
      // **頭上の高い球だけ**。通常のスマッシュの線（PLAYER.SMASH_MIN_Y=1.8）より上に引いて
      // あり、普通のリーチの上限（PLAYER.REACH_Y=2.5）にも迫る＝下の REACH_Y_BONUS で
      // 天井を 2.95 まで上げて、初めて気持ちよく届く高さ。胸の高さの速い球や、少し浮いた
      // だけの球ではこの線に届かないので、決め球としての場面が濁らない。
      MIN_Y: 2.2,
      // 「前へ踏み込みながら叩く」技なので、ネット方向(+z)へこの速さ以上で動いていること。
      // 止まって待つ普通のスマッシュとの違いはここだけで、**溜めは要らない**
      // （通常のスマッシュが要求する PLAYER.SMASH_MIN_CHARGE は、この技には掛からない）。
      MIN_FWD: 2.2,
      REACH_Y_BONUS: 0.45,   // 打てる高さの上限（PLAYER.REACH_Y）をこれだけ引き上げる
      T_MULT: 0.66,          // SHOT.SMASH_T に掛ける（＝通常のスマッシュよりさらに速い）
      // 初速の上限(m/s)。SHOT.SMASH_T は距離に関わらず一定の飛翔時間なので、そのままだと
      // 「ベースライン後方から相手のベースラインまで20m」を 0.23秒＝287km/h で飛ばす、
      // 当たった瞬間に消えて見える球になる（実測）。狙いが遠いときだけ飛翔時間を伸ばして
      // ここで頭打ちにする。75m/s ≒ 270km/h（実際のテニスのスマッシュの最速記録と同じくらい。
      // 技として「いちばん速い決め球」であることがはっきりする速さ）。
      MAX_SPEED: 75,
      Z: 11.0,               // 着地の深さ（ベースライン際へ突き刺す）
      CLEARANCE: 0.05,       // ネットすれすれを叩き落とす（＝角度が立つ）
      JUMP_MULT: 1.75,       // 見た目のジャンプの高さ（SWING.SMASH_JUMP_H に掛ける）
    },

    /** 飛びつきボレー：リーチが伸びる代わりに、打った後の硬直が長い。 */
    DIVE: {
      REACH_MULT: 1.95,      // この1打だけ PLAYER.REACH がこの倍率まで伸びる
      X: 4.05,               // サイドライン際への角度（HALF_W=4.115 のすぐ内側）
      Z: 2.1,                // ネット際に短く落とす
      T: 0.30,               // 飛翔時間（VOLLEY.ANGLE_T より速い）
      RECOVER: 0.95,         // 打った後に動けない時間(秒)。PLAYER.HIT_RECOVER_DELAY の代わり
      // 見た目の倒れ込み(rad)。1.2 前後まで倒すとカメラ（選手の後方）からはうつ伏せに
      // 地面へ突っ込んで見えるので、「斜めに跳んで伸びている」と読める範囲に留める。
      LEAN: 0.95,
      LIFT: 0.42,            // 飛びついた瞬間に浮く高さ(m)。倒れ込みと合わせて宙に浮かせる
    },

    /** 縮地：ボールの打点へ瞬間移動する。打つ球そのものは普通のショット。 */
    DASH: {
      LEAD_T: 1.25,          // 何秒先の打点まで探すか（WINDOW より長くしない：跳んだ先で
                             // 振り終わってしまい、待っているだけの空振りになるため）
      WINDOW: 1.45,          // 瞬間移動した後、スイングが有効な時間の上限(秒)。通常は PLAYER.SWING_WINDOW
      WINDOW_MARGIN: 0.12,   // 打点までの時間にこれだけ足した長さを有効時間にする
      BACK_OFF: 0.25,        // 打点のわずかに手前（自陣側）に立つ(m)
      // 発動に必要な「ボールとの距離」の下限(m)。ラケットの届く範囲（PLAYER.REACH＝1.55m）の
      // 倍くらい離れていて初めて「取れない球」と呼べる、という線引き。これを見ないと、
      // すぐ横を速く通り過ぎる球——手を伸ばせば届きそうな「ギリギリ届かない」球——にも
      // 出てしまう（そういう球でも2バウンド目は遠いので dashUnreachable は成立する）。
      MIN_DIST: 3.0,
      // 「走っても間に合わない」と見なす基準（game.js#dashUnreachable）。次の着地までに
      // 全力で走れる距離（PLAYER.SPEED × 残り時間）のこの割合より遠いときだけ発動できる。
      // これがないと、溜めを少し早く離しただけの普通の球（＝いま届かないだけで、走れば
      // 余裕で届く球）まで縮地の対象になり、他の技がほとんど出なくなる。
      RUNNABLE: 0.95,
      FX_T: 0.42,            // 残像が消えるまでの時間(秒)
      FX_OPACITY: 0.45,      // 残像の最大不透明度
    },

    /** ツイーナー（股抜き）：背走したまま股下から抜くロブ返し。 */
    TWEENER: {
      BEHIND: 0.15,          // ボールが自分よりこれだけ後ろ（ベースライン側）にあれば発動できる(m)
      REACH_MULT: 1.35,      // 背中側の球なので、この1打だけリーチを伸ばす
      MIN_Y: 0.2,            // 低すぎる（転がっている）球には出せない
      Z: 11.2,               // 相手のベースライン際へ落とす
      T: 1.18,               // ロブなので山なり
      CLEARANCE: 1.7,        // 詰めてきた相手の頭上を越す高さ
      X: 1.6,                // 左右の振り幅（背面打ちなので大きくは狙えない）
    },

    /**
     * ドライブボレー：ノーバウンドの**浮き球**を、ストローク並みの速さで叩く。
     * 高さの上下で場面を区切ってある：腰より上に浮いた球（MIN_Y）から、頭上に上がって
     * スマッシュになる高さ（MAX_Y）の手前まで。上を切らないと、スマッシュやダンクの
     * 場面（＝上から叩き下ろす球）まで横振りのドライブボレーが拾ってしまう。
     */
    DRIVE: {
      MIN_Y: 0.95,           // この高さ以上に浮いたノーバウンドの球にだけ出せる
      // ここから上はスマッシュの領分なので出さない。通常のスマッシュになる線
      // （PLAYER.SMASH_MIN_Y）と同じ値を参照する＝2つが食い違うことがない。
      MAX_Y: PLAYER.SMASH_MIN_Y,
      X: 3.4,                // 狙う左右（ライン際まで振り切らない代わりに確実）
      Z: 9.8,                // 深く突き刺す
      T: 0.40,               // 飛翔時間（SHOT.CHARGE_T よりやや遅い程度）
      CLEARANCE: 0.22,
    },

    /**
     * バギーホイップ：走らされた体勢から、超トップスピンで急角度のクロスへ。
     * **V（トップスピン）で溜めたフォアハンドのときだけ**出る（大きく擦り上げて振り抜く
     * 打ち方そのものが技なので、フラット／スライスやバックハンドでは成立しない）。
     * 曲がる向きも打ち方で決まっていて常に同じ＝画面の右から左（world の +x 方向。
     * カメラの都合で world +x が画面の左に映る）。狙う側も曲がる先に固定される
     * （←→ では変えられない）。
     */
    BUGGY: {
      MIN_SPEED: 2.2,        // これ以上の速さで動いている（＝まだ止まりきっていない）
      // 「振り回されて追いついた」ときだけの技にするための2条件（どちらもラケット側＝
      // 画面の右へ向かう量。game.js が RACKET_SIDE.you を掛けて符号を吸収する）。
      // ここを緩くすると、ちょっと横に動いただけの普通のフォアでも出てしまう。
      MIN_RUN_X: 2.5,        // 相手が打ってから、右へこれだけ走ってきたこと(m)
      MIN_X: 2.6,            // 打つ位置がセンターからこれだけ右に寄っていること(m。サイドラインは 4.12)
      // クロス（既定。←→ が無入力か、自分と反対側を向いているとき）
      X: 4.0,                // 落とすサイドライン際（曲がった先。符号は常に +＝画面の左）
      Z_MIN: 4.0,            // ショートクロス（角度をつけるぶん浅い）
      Z_MAX: 6.2,
      T: 0.62,               // 山なりに見えて速い（トップスピンの実効重力で急降下する）
      CLEARANCE: 0.85,       // ネットのかなり上を通す
      /**
       * ストレート（←→ で自分のいる側を選んだとき）。**ネットポストの外側を回して**
       * サイドライン際へ戻す（実際のテニスの「ポールを回すショット」）。曲がりの向きは
       * クロスと同じ（コート中央へ向かう）で、そのぶん外へ膨らんでから戻ってくる。
       * 必要な横加速度は打点とポストの位置関係から game.js#aroundPostCurve() が逆算する。
       * MAX_CURVE を超える（＝コートの内側すぎてポールを回りきれない）ときは、
       * ふつうの曲がるストレート（CURVE）として打つ。実測では打点が world -4.6m あたり
       * ＝シングルスのサイドライン(4.12m)の外まで追い出されて初めてポールを回れる。
       */
      LINE_X: 3.85,          // 落とすサイドライン際（自分のいる側）
      LINE_Z_MIN: 7.4,       // 深さ。深いほどネット通過が早く＝ポールを回りやすい
      LINE_Z_MAX: 9.2,
      LINE_T: 0.66,
      LINE_CLEARANCE: 0.35,  // ポストの外を通るときは「地面からの高さ」の余裕になる
      POST_CLEAR: 0.30,      // ポスト（COURT.NET_HALF）のこれだけ外側を通す(m)
      MAX_CURVE: 45,         // ポールを回すのに掛けてよい横加速度の上限(m/s²)
      // 飛翔中ずっと横向きに掛かる加速度(m/s²)。狙う側（＝曲がる先）へ効くので、
      // solveShot() は「曲がるぶんを見越して内側へ打ち出す」初速を返す＝ほぼ真っ直ぐ
      // 飛び出してから、サイドライン際へ弧を描いて曲がり落ちる（着地点は狙いのまま）。
      // 向きは常に +（world +x ＝ 画面の左）＝「画面の右から左へ曲がる」。
      // 実測（ベースラインからショートクロス、飛翔0.62秒）：打点と着地点を結ぶ直線から
      // いちばん膨らむところで 6→0.50m／10→0.86m／16→1.35m／18→1.52m。16 なら打ち出しの
      // 横方向の初速がほぼ0（＝真っ直ぐ出てから曲がる）になり、曲がりが一番読み取りやすい。
      // バウンドすると摩擦で消える（physics.reflectBounce）。
      CURVE: 16,
    },

    /**
     * 鷹の目：足を止めて狙い澄ました1打。狙いのブレが完全に消え、ライン際に落ちる。
     * 自動発動なので、つなぎのブロック返球まで巻き込まないよう「ある程度溜めた1打」に限る
     * （MIN_CHARGE）。他の技は打点の高さや位置そのものが条件なので、この制限は要らない。
     */
    HAWK: {
      MIN_CHARGE: 0.5,       // これ以上溜めて離した1打だけ（0〜1）
      INSET: 0.12,           // サイドラインからこれだけ内側に落とす(m)
      Z_MIN: 8.4,            // 溜めなしでもこの深さ
      Z_MAX: 11.0,           // フル溜めならベースライン際
      T_MULT: 0.92,          // 通常のストロークより気持ち速い
    },
  };

  /**
   * 必殺技の一覧。**この配列の並び順がそのまま優先度**（上ほど先に判定され、条件を
   * 満たした最初のひとつだけが発動する）。スタート画面の並び順でもある。
   * - key    game.js / hud.js が使う識別子。SPECIAL の小オブジェクト名と対応
   * - label  画面に出す技名
   * - short  HUD の残り回数表示に使う短い名前（横に並べるので長いと折り返す）
   * - when   どんな場面で出るか（スタート画面とHUDの説明に使う）
   * - hint   何が起きるか
   */
  const SPECIAL_MOVES = [
    {
      ...
      short: 'キック',
      when: '自分のサーブ',
      hint: '高く跳ねて相手を押し下げる。溜めすぎてもフォールトしない',
    },
    {
      key: 'dunkSmash',
      label: 'ダンクスマッシュ',
      short: 'ダンク',
      when: '前に詰めながら打つ、頭上の高いノーバウンドの球',
      hint: '打点の上限が上がり、真下へ叩き落とす最速の決め球（270km/h）。溜めは要らない',
    },
    {
      key: 'shukuchi',
      label: '縮地',
      short: '縮地',
      when: '届かない球',
      hint: 'ボールの打点へ瞬間移動する。返せない球を返しにいく',
    },
    {
      key: 'tweener',
      label: 'ツイーナー',
      short: 'ツイーナー',
      when: '球に抜かれたとき',
      hint: '背走したまま股下から抜くロブ。詰めてきた相手の頭上を越す',
    },
    {
      key: 'divingVolley',
      label: '飛びつきボレー',
      short: '飛びつき',
      when: 'ネット前のノーバウンド',
      hint: 'リーチが倍近く伸びて飛びつく。決まりやすいが硬直が長い',
    },
    {
      key: 'driveVolley',
      label: 'ドライブボレー',
      short: 'ドライブ',
      when: '腰から頭の高さに浮いたノーバウンド',
      hint: 'ノーバウンドのまま強打で深く突き刺す',
    },
    {
      key: 'buggyWhip',
      label: 'バギーホイップ',
      short: 'バギー',
      when: '走らされているとき',
      hint: '超トップスピンで急角度のショートクロスへ。狙いはブレない',
    },
    {
      key: 'hawkEye',
      label: '鷹の目',
      short: '鷹の目',
      when: '足が止まっているとき',
      hint: '狙いのブレが完全に消え、ライン際ぎりぎりに落ちる',
    },
  ];

  /** スタート画面の「おすすめ」プリセット（Z キーでの切り替えに使う）。 */
  const SPECIAL_PRESET = ['kickServe', 'dunkSmash', 'shukuchi', 'divingVolley'];
  ...
  const SERVE = {
    ...
    // セカンドサーブ（CPU/AI 専用。人間は溜め量とコース入力で自分で加減する）。
    // 以前は serve() が this.serveNumber を球速にも狙いにも一切使っておらず、AI は1本目も
    // 2本目も同じフル威力のフラット系サーブを打っていた（＝「2本目まで1本目と同じ速さで
    // 来る」というユーザー報告の直接の原因）。実際のテニスと同じく「1本目より2割ほど遅く、
    // ラインから余裕を取り、回転をかけて確実に入れにいく」1本にする。
    SECOND_T_MULT: 1.28,        // 飛翔時間の倍率（大きいほど遅い。球速およそ -22%）
    SECOND_FLAT_CHANCE: 0.15,   // 2本目はほとんど回転をかける（CPU.SPIN_FLAT_CHANCE の代わり）
    SECOND_TOP_SHARE: 0.75,     // 回転をかけるとき、この割合でトップスピン（＝キックサーブ）
    SECOND_AIM_WIDE_MAX: 3.45,  // ワイドもサイドラインから余裕を取る（角度サーブは選ばない）
    SECOND_DEPTH_MIN: 0.9,      // 深さもサービスラインから余裕を取る（小さいほど深い）
    SECOND_DEPTH_MAX: 2.6,
    SECOND_NET_MULT: 0.45,      // ネットに掛ける確率（NET_CHANCE）もこのぶん下がる
    // CPU/AI がサーブを外す確率。実際のテニスでは1本目は6割強しか入らず、だからこそ
    // 思い切り攻めた1本が打てる。以前は幾何的に外れるぶん（ネット掛かり＋ワイド）しか
    // なくて1本目も2本目も約9割入っており、「毎回フル威力の1本目がほぼ確実に入る」
    // ＝球速そのものは現実的でもサーブがきつすぎる、という状態だった。
    // 抽選に当たった1本は、人間の溜めすぎ（serveFaultChance()）とまったく同じ外し方で
    // 狙いをサービスボックスの外へずらす＝画面にロング／サイドアウトがちゃんと出る。
    // 値は、幾何的に外れるぶんと掛け合わせた最終的な成功率が 1本目およそ62%・2本目およそ
    // 92%（＝ダブルフォールトは全サービスポイントの約3%）になるよう決めてある。
    // ATPの平均は 1st 62%／2nd 92%／DF 3%前後で、そこに合わせた。
    CPU_FIRST_MISS: 0.30,
    CPU_SECOND_MISS: 0.05,
  };
  ...
  const AUDIO = {
    ...
    SPECIAL: {
      ...
      BRUSH_VOL: 0.05, BRUSH_HZ: 3200, BRUSH_DUR: 0.22,
    },
  ...
  const TIMING = {
    // CPU/AI のサーブは「構える → 間を置いてトス（CPU_SERVE_READY）→ 打つ（CPU_SERVE_DELAY）」
    // の2段。以前は READY が無く、ポイントが始まった瞬間にトスが上がって 0.9秒後には球が
    // 飛んできていたため、レシーバー（人間）に構える間がなく「サーブが早すぎる」と
    // 感じられていた（ユーザー報告）。実際のテニスでは打つ前に必ず一拍ある。
    // CPU_SERVE_DELAY はトスを上げてから打つまでなので、トスの滞空時間（SERVE.BALL_Y →
    // TOSS_PEAK → BALL_Y でおよそ1.2秒）より必ず短くしておくこと。長いと打つ前にボールが
    // 手元へ落ちきってしまう（update() の保険が拾うが、トスが2回上がったように見える）。
    CPU_SERVE_READY: 0.9,
    CPU_SERVE_DELAY: 0.9,
    NEXT_POINT: 1.5,
    NEXT_MATCH: 2.6,
  ...
  const THEME = {
    ...
    SKIN: 0xecc6a0,
    GRIP: 0x22303f,
    // 縮地の残像（跳ぶ前に立っていた位置に一瞬だけ残る半透明の分身）。必殺技まわりの
    // 表示（HUDのゲージ・残り回数）と同じ金色で揃える。
    DASH_GHOST: 0xffc83d,
  ...
  const SWING = {
    ...
    /**
     * ツイーナー（必殺技の股抜き）。相手に背を向けたまま打つので横振り(rotation.y)は
     * ほぼ使わず、仰角(rotation.z)を真下（-π/2 付近）から前方へ通す。前後の傾き
     * (rotation.x)で腕を体の内側＝股の下へ入れる。
     */
    TWEENER: {
      Y: -0.15,
      Z_START: -1.45, // ラケットが股の下（ほぼ真下）
      Z_END: -0.45,   // 低いまま前方へ振り抜く（上まで振り上げると股抜きに見えない）
      X: -0.55,       // 体の内側へ入れる
    },
    // 飛びつきボレー（必殺技）。体ごと打つ側へ倒れ込み、脚を後ろへ流す。
    DIVE_LEG_TRAIL: 0.95,  // 後ろへ流す脚の角度(rad)
    DIVE_KNEE_TUCK: 0.55,  // そのときの膝の曲げ(rad)
    DIVE_TORSO_X: 0.55,    // 体幹の前傾(rad)
  };
